...
class URLEncoderTests: XCTestCase {
    ...
    func test_encodeWikipediaURL_withNegativeCoordinates_returnsCorrectlyEncodedURL() {
        let sut = makeSUT()
        let latitude = -33.8688
        let longitude = 151.2093
        
        let encodedURL = sut.encodeWikipediaURL(latitude: latitude, longitude: longitude)
